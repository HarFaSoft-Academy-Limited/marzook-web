...
export function EditParentDialog({ parent, hideModal, showModal }: any) {
  ...
  const handleUpdateParent = async () => {
    const res = await updateParent(parent.id, formData);
    if (res) {
      hideModal(false);
    } else {
    ...
    <Dialog open={showModal} onOpenChange={hideModal}>
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Update parent information in the system.</DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button className="bg-green-600 hover:bg-green-700" onClick={handleUpdateParent}>
            Update Parent
          </Button>
        </DialogFooter>
      </DialogContent>
